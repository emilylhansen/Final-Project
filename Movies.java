...
//      else {
  ...
  public void deleteBST(Movie p){
    if (isEmptyTree() == false){
      //if root is p, delete
      if (root.getDate() == p.getDate()){
        deleteRoot(p);
      }
      //delete other node in bst
      else {
  ...
  private void delete2(Movie root, Movie p){
    Movie temp = root;
      ...
      //no children
        ...
        if (findParent(p).getLeft().getDate() == p.getDate()){
          p.setLeft(null);
        }
        else if (findParent(p).getRight().getDate() == p.getDate()){
          p.setRight(null);
        }
      }
      //two children
